...
class InferenceDataset(torch.utils.data.Dataset):
    def __init__(self, test_dir, target_sources, file_filter="mixture"):
        self.test_dir = Path(test_dir)
        self.target_sources = target_sources
        file_filter = file_filter if file_filter else "*"
        wav_files = glob(os.path.join(test_dir, f"**/{file_filter}.wav"), recursive=True)
        flac_files = glob(os.path.join(test_dir, f"**/{file_filter}.flac"), recursive=True)
